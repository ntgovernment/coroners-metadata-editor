...
│   └── editor.css              # Custom CSS — edit affordances, status colours, tooltips
│
...
- **`row-template.html`** is a Matrix _Design_ / _Paint Layout_ fragment. Squiz replaces `%asset_*%` tokens with live asset data at render time. The hidden `<td class="hidden">` with the year value is used by DataTables for chronological sorting.
...
- **`src/editor.css`** styles the edit-affordance tooltips (`::before` / `::after` pseudo-elements with Font Awesome icon + "Edit {label}" text), per-status row colours, and the fixed-position save-result toast.
